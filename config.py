...
chpwd=False
if (not config['sina']['password']) or query_yn('change passwords? (yes/no)%s:','no'):
  password=getpass.getpass('initialize admin password:')
  chpwd=True
  if password!=getpass.getpass('input password again:'):
